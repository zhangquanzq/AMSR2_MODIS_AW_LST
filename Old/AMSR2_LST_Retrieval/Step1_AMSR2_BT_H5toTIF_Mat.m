...
for i = 1 : length(amsr2BandFolderList)
    ...
    for j = 1 : length(amsr2YearFolderList)
        ...
        for k = 1 : length(amsr2MonthFolderList)
            ...
            fprintf('转换%s年%s月的数据格式.\n', amsr2YearFolderName, amsr2MonthFolderName)

            % 读取AMSR2 H5每日文件列表.
            amsr2H5DailyList = dir(fullfile(amsr2H5MonthFolderPath, '*.h5'));
            amsr2H5DailyList = {amsr2H5DailyList.name}';
            for ii = 1 : length(amsr2H5DailyList)
                amsr2H5DailyName = amsr2H5DailyList{ii};
                amsr2H5DailyPath = fullfile(amsr2H5MonthFolderPath, amsr2H5DailyName);

                % 保存H5格式中的水平极化亮温数据为TIF格式.
                amsr2BtHTifDailyName = replace(amsr2H5DailyName, '.h5', '_BtH.tif');
                amsr2BtHTifDailyPath = fullfile(amsr2TifMonthFolderPath, amsr2BtHTifDailyName);
                if ~exist(amsr2BtHTifDailyPath, 'file')
                    try
                        btHArray = h5read(amsr2H5DailyPath, '/Brightness Temperature (H)')';
                        btHArray = btHArray(startLat : endLat, startLon : endLon);
                        geotiffwrite(amsr2BtHTifDailyPath, btHArray, amsr2Ref,CoordRefSysCode=4326);
                    catch
                        disp(['有问题数据：', amsr2BtHTifDailyName]);
                    end
                end

                % 保存H5格式中的垂直极化亮温数据为TIF格式.
                amsr2BtVTifDailyName = replace(amsr2H5DailyName, '.h5', '_BtV.tif');
                amsr2BtVTifDailyPath = fullfile(amsr2TifMonthFolderPath, amsr2BtVTifDailyName);
                if ~exist(amsr2BtVTifDailyPath, 'file')
                    try
                        btVArray = h5read(amsr2H5DailyPath, '/Brightness Temperature (V)')';
                        btVArray = btVArray(startLat : endLat, startLon : endLon);
                        geotiffwrite(amsr2BtVTifDailyPath, btVArray, amsr2Ref,CoordRefSysCode=4326);
                    catch
                        disp(['有问题数据：', amsr2BtVTifDailyName]);
                    end
                end
            end
        end
