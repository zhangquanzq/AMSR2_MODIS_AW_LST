...
%   means Quadratic.
flg3 = 2;
% Flag control whether export the zonal AMSR2 BT and MODIS LST iamges and their scatterplots.
%   1 means Yes, 0 means No.
flg4 = 1;
% Flag assign the regression strategy. 1 means Year, 2 means Season, 3 means Month, 4 means
%   optimized model.
flg5 = 4;
% Flag assgin only get the regression coefficients or also retrieve the masked and full extent AMSR2
...
flg7 = 1;
% Flag assign the resolutions. 1 means 10. 2 means 25.
flg8 = 1;

% Flag checking.
if flg5 == 4 && flg6 == 2
    message = ['The retrieving of AMSR2 LST using single model is not supported when optimized', ...
        ' model was appointed!'];
   error(message); 
end
...
% The orbit of AMSR2 satellite, also indicates the day and night. A means day, D means night.
orbits = {'A', 'D'};
dayNight = {'Day', 'Night'};
Resolution = {'10', '25'};
% The percentage threshold of avaliable Desert, Water, Glacier, Buiding, or Snow pxin the
%   extent of one AMSR2 pixel.
snowThreshold = 0.6;
lcThreshold = 0.6;
% The channel of AMSR2 BT that is combined with MODIS LST to plot scatter.
channels = {'06h', '06v', '07h', '07v', '10h', '10v', '18h', '18v', '23h', '23v', '36h', '36v', '89h', '89v'};
channelsN = length(channels);
% The variable number involved in the regerssion model.
variablesN = channelsN + 3; % (14 channels) + (2 quadratic term) + (1 intercept).
% Season months.
...
%% Paths.
% Root Paths.
addpath('F:\2016\Code\Matlab_AMSRE_LST_Retrieval\Functions\');
dataRootPath = 'F:\2016\AMSR2_Retrivel_2016\Data\';
figureRootPath = 'F:\2016\AMSR2_Retrivel_2016\Figure\';
% ------------------------------------------ Input paths -------------------------------------------
% AMSR2 BT images.
if flg8 == 1
    amsreGridFolder = 'AMSR2_OneTenthDegreeGrid\';
elseif flg8 == 2
    amsreGridFolder = 'AMSR2_QuarterDegreeGrid\';
end
amsreBtCnFolder = 'AMSR2_3_BT_ClipTP_Tif\';
amsreBtMaskFolder = 'AMSR2_5_BT_MaskTP_Tif\';
amsreYearFolder = ['AMSR2_', testYears{flg7}, 'XXXX\'];
...
modisLstCnFolder = 'MYD11A1_3_LST_ClipTP_TIF\';
if flg8 == 1
    modisLstMaskFolder = 'MYD11A1_5_LST_MaskTP_TIF_10\';
elseif flg8 == 2
    modisLstMaskFolder = 'MYD11A1_5_LST_MaskTP_TIF_25\';
end
...
modisSnowCnFolder = 'MYD10C1_2_Snow_TP_TIF\';
...
modisLcPath = [dataRootPath, modisLcFolder, 'MCD12Q1.A2016_gcs_resmpl_reclsfy_clip_resample1.tif'];
desertPath = [dataRootPath, modisLcFolder, 'Desert_TP_gcs2.tif'];

% Zone image.
zonesFolder = 'Zones\';
if flg8 == 1
    zonesFileNameList = {'GeographicalZones_merge_10.tif'};
%     zonesFileNameList = {'GeographicalZones_merge_10.tif', 'LandCoverZones.tif'};
else
    zonesFileNameList = {'GeographicalZones_merge9.tif'};
%     zonesFileNameList = {'GeographicalZones_merge9.tif', 'LandCoverZones.tif'};
end
zonesFilePath = [dataRootPath, zonesFolder, zonesFileNameList{flg2}];

% ----------------------------------------- Output paths -------------------------------------------
% Matlab variables.
matlabVariableFolder = 'Matlab\AMSR2_LST_Retrieval\';
matlabVariablePath = [dataRootPath, matlabVariableFolder];
if ~exist(matlabVariablePath, 'dir')
    mkdir(matlabVariablePath);
end

% Zonal AMSR2 BT image.
outAmsreBtTifFolder = 'AMSR2_6_BT_Zonal_TIF\';
...
% Retreived AMSR2 LST image.
outAmsreLstTifFolder = 'AMSR2_7_LST_TIF\';
...
%% Data Lists.
% Read the complete and masked AMSR2 daily folder list.
amsreBtCnDayFolderList = dir([amsreBtCnYearPath, 'AMSR2*']);
amsreBtCnDayFolderList = {amsreBtCnDayFolderList.name}';
amsreBtCnDayFolderListN = length(amsreBtCnDayFolderList);

amsreBtMaskDayFolderList = dir([amsreBtMaskYearPath, 'AMSR2*']);
...
% Check whether the number of masked AMSR2 BT and MODIS LST images are the same.
if (amsreBtMaskDayFolderListN ~= modisLstMaskDayListN)
    error('The number of masked AMSR2 BT and MODIS LST images are different!');
end
...
% Read the spatial reference of complete AMSR2 BT images. The spatial references of all AMSR2 BT
%   images are the same.
[~, btReference] = geotiffread([amsreBtCnYearPath, amsreBtCnDayFolderList{1}, ...
    '\AMSR2_D', Resolution{flg8}, '_',  testYears{flg7}, '0101', orbits{flg1}, '_06H.tif']);
...
% Read the spatial reference of masked AMSR2 BT images and MODIS LST images. The spatial references
%   of all the masked AMSR2 BT images and MODIS LST iamges are the same.
...
% --------------------------------------------------------------------------------------------------
% Get the starting/ending row numbers and column numbers from the complete AMSR2 BT images that are
%   aligned with the boundary of AMSR2 BT and MODIS LST images.
if btLstXMin > btXMin
...
%   that are aligned with the boundary of the first pixel at the up-left corner of AMSR2 BT and
%   MODIS LST images.
for ii = 1 : btLstColN
...
%   with the boundary of AMSR2 BT and MODIS LST images.
...
%% Read Data.
% Create 3D arrays to restore the AMSR2 BT, MODIS LST, and MODIS Snow images in the year, skip the
%   corresponding layer if there is no image in certain day.

% Read the complete AMSR2 BT images from the 12 channels in the operational year.
outAmsreBtCnYearArrayMatPath = [matlabVariablePath, '1_AMSR2_BT_TP_', dayNight{flg1}, '_', ...
    testYears{flg7}, '.mat'];
if ~exist(outAmsreBtCnYearArrayMatPath, 'file')
    [amsreBt06hCnYearArray, amsreBt06vCnYearArray, amsreBt07hCnYearArray, amsreBt07vCnYearArray, amsreBt10hCnYearArray, ...
   ...
   for i = 1 : yearDateListN
        ...
        % Read the AMSR2 BT images in the day that the AMSR2 BT images are avaliable.
        ...
        disp(['Read complete AMSR2 BT image: ', amsreFolderName]);
        amsreBtCnDayPath = [amsreBtCnYearPath, amsreFolderName, '\'];
        amsreBtCnChannelList = dir([amsreBtCnDayPath, 'AMSR2_D*', orbit{flg1}, '*H.tif']);
        amsreBtCnChannelList1 = dir([amsreBtCnDayPath, 'AMSR2_D*', orbit{flg1}, '*V.tif']);
        amsreBtCnChannelList = {amsreBtCnChannelList.name}';
        amsreBtCnChannelList1 = {amsreBtCnChannelList1.name}';
        amsreBtCnChannelListN = length(amsreBtCnChannelList)*2; % Remove AMSR2 TIM image.
        amsreBtCnArray = zeros(btLstRowN, btLstColN, amsreBtCnChannelListN);
        for j = 1 : amsreBtCnChannelListN/2
            amsreBtCnLayer = geotiffread([amsreBtCnDayPath, amsreBtCnChannelList{j}]);
            amsreBtCnLayer = amsreBtCnLayer(btStartRow-1 : btEndRow, btStartCol-1 : btEndCol);
            amsreBtCnLayer(amsreBtCnLayer == 65534) = nan;
            amsreBtCnArray(:, :, j) = double(amsreBtCnLayer) * 0.01;
        end
        
        for j = amsreBtCnChannelListN/2 + 1 : amsreBtCnChannelListN
            amsreBtCnLayer = geotiffread([amsreBtCnDayPath, amsreBtCnChannelList1{j - amsreBtCnChannelListN/2}]);
            amsreBtCnLayer = amsreBtCnLayer(btStartRow-1 : btEndRow, btStartCol-1 : btEndCol);
            amsreBtCnLayer(amsreBtCnLayer == 65534) = nan;
            amsreBtCnArray(:, :, j) = double(amsreBtCnLayer) * 0.01;
        end
        ...
        amsreBt06vCnYearArray(:, :, i) = amsreBtCnArray(:, :, 8);
        amsreBt07hCnYearArray(:, :, i) = amsreBtCnArray(:, :, 2);
        amsreBt07vCnYearArray(:, :, i) = amsreBtCnArray(:, :, 9);
        amsreBt10hCnYearArray(:, :, i) = amsreBtCnArray(:, :, 3);
        amsreBt10vCnYearArray(:, :, i) = amsreBtCnArray(:, :, 10);
        amsreBt18hCnYearArray(:, :, i) = amsreBtCnArray(:, :, 4);
        amsreBt18vCnYearArray(:, :, i) = amsreBtCnArray(:, :, 11);
        amsreBt23hCnYearArray(:, :, i) = amsreBtCnArray(:, :, 5);
        amsreBt23vCnYearArray(:, :, i) = amsreBtCnArray(:, :, 12);
        amsreBt36hCnYearArray(:, :, i) = amsreBtCnArray(:, :, 6);
        amsreBt36vCnYearArray(:, :, i) = amsreBtCnArray(:, :, 13);
        amsreBt89hCnYearArray(:, :, i) = amsreBtCnArray(:, :, 7);
        amsreBt89vCnYearArray(:, :, i) = amsreBtCnArray(:, :, 14);
    end
    save(outAmsreBtCnYearArrayMatPath, 'amsreBt06hCnYearArray', 'amsreBt06vCnYearArray', 'amsreBt07hCnYearArray', 'amsreBt07vCnYearArray',...
    ...
    lia = ismember({'amsreBt06hCnYearArray', 'amsreBt06vCnYearArray', 'amsreBt07hCnYearArray', 'amsreBt07vCnYearArray','amsreBt10hCnYearArray',...
        'amsreBt10vCnYearArray', 'amsreBt18hCnYearArray', 'amsreBt18vCnYearArray', ...
        'amsreBt23hCnYearArray', 'amsreBt23vCnYearArray', 'amsreBt36hCnYearArray', ...
...
% Read the masked AMSR2 BT and MODIS LST images in the operational year.
outBtLstMaskYearArrayMatPath = [matlabVariablePath, '2_AMSR2_BT_MODIS_LST_Mask_', ...
    dayNight{flg1}, '_', testYears{flg7}, '.mat'];
if ~exist(outBtLstMaskYearArrayMatPath, 'file')
    [amsreBt06hMaskYearArray, amsreBt06vMaskYearArray, amsreBt07hMaskYearArray, amsreBt07vMaskYearArray, amsreBt10hMaskYearArray, ...
    ...
    % The number of days of the maksed AMSR2 BT and MODIS LST images are the same.
    parfor i = 1 : yearDateListN
        ...
        % Get index of the day in which the AMSR2 BT and MODIS LST images are avaliable.
        [~, btLstDayIndex] = ismember(yearDateList{i}, modisLstMaskDateList);
        if btLstDayIndex == 0, continue; end
        disp(['Read masked AMSR2 BT and MODIS LST images: ', modisLstMaskDateList{btLstDayIndex}]);
        ...
        % Read the masked AMSR2 BT images in 14 channels.
        ...
        amsreBtMaskChannelList = dir([amsreBtMaskDayPath, 'AMSR2_D*', orbit{flg1},'*H.tif']);
        amsreBtMaskChannelList1 = dir([amsreBtMaskDayPath, 'AMSR2_D*', orbit{flg1},'*V.tif']);
        amsreBtMaskChannelList = {amsreBtMaskChannelList.name}';
        amsreBtMaskChannelList1 = {amsreBtMaskChannelList1.name}';
        amsreBtMaskChannelListN = length(amsreBtMaskChannelList)*2;
        amsreBtMaskArray = zeros(btLstRowN, btLstColN, amsreBtMaskChannelListN);
        for j = 1 : amsreBtMaskChannelListN/2
            amsreBtMaskArray(:, :, j) = ...
                geotiffread([amsreBtMaskDayPath, amsreBtMaskChannelList{j}]);
        end
        for j = amsreBtMaskChannelListN/2 + 1 : amsreBtMaskChannelListN
            amsreBtMaskArray(:, :, j) = ...
                geotiffread([amsreBtMaskDayPath, amsreBtMaskChannelList1{j - amsreBtMaskChannelListN/2}]);
        end
      
        amsreBtMaskArray(amsreBtMaskArray < 0) = nan;
        
        % Get the index layer and array used for masking the overlapped region of AMSR2 BT and MODIS
        %   LST images. The masked AMSR2 BT and MODIS LST images should already have the same
        ...
        % Masking the overlap of MODIS LST and AMSR2 BT images.
        ...
        amsreBt06vMaskYearArray(:, :, i) = amsreBtMaskArray(:, :, 8);
        amsreBt07hMaskYearArray(:, :, i) = amsreBtMaskArray(:, :, 2);
        amsreBt07vMaskYearArray(:, :, i) = amsreBtMaskArray(:, :, 9);
        amsreBt10hMaskYearArray(:, :, i) = amsreBtMaskArray(:, :, 3);
        amsreBt10vMaskYearArray(:, :, i) = amsreBtMaskArray(:, :, 10);
        amsreBt18hMaskYearArray(:, :, i) = amsreBtMaskArray(:, :, 4);
        amsreBt18vMaskYearArray(:, :, i) = amsreBtMaskArray(:, :, 11);
        amsreBt23hMaskYearArray(:, :, i) = amsreBtMaskArray(:, :, 5);
        amsreBt23vMaskYearArray(:, :, i) = amsreBtMaskArray(:, :, 12);
        amsreBt36hMaskYearArray(:, :, i) = amsreBtMaskArray(:, :, 6);
        amsreBt36vMaskYearArray(:, :, i) = amsreBtMaskArray(:, :, 13);
        amsreBt89hMaskYearArray(:, :, i) = amsreBtMaskArray(:, :, 7);
        amsreBt89vMaskYearArray(:, :, i) = amsreBtMaskArray(:, :, 14);
    end
...
lcCodeList = unique(lcLayer);  % [-128000, 1000, 2000, 3000, 4000, 5000]
lcCodeList = lcCodeList(2:end);  % [1000, 2000, 3000, 4000, 5000]
...
zonesLayer(ismember(zonesLayer, 127)) = nan;
...
for ii = 1 : btLstRowN
    for jj = 1 : btLstColN
        ...
        % If any of the land cover doesn't exceed 60% in one AMSR2 pixel, treat it as a mixed pixel.
...
if ~exist(outLcPercentArrayMatPath, 'file')
    ...
    for i = 1 : yearDateListN
        % Techniques handeling the issues in parallel computing.
        tempZonesLcLayer = zonesLcLayer(2:146,1:313);
...
if ~exist(outRegressionParametersMatPath, 'file')    
    ...
    for n = 1 : pureLcIdN
        ...
        % Pick out the zonal AMSR2 BT and MODIS LST images from the array of the whole year.
        ...
        amsreBt07hMaskZoneYearArray = setnan(amsreBt07hMaskYearArray, ~zonesLcIndexArray);
        amsreBt07vMaskZoneYearArray = setnan(amsreBt07vMaskYearArray, ~zonesLcIndexArray);        
        ...
        if flg6 == 2
            ...
            amsreBt07hCnZoneYearArray = setnan(amsreBt07hCnYearArray, ~zonesLcIndexArray);
            amsreBt07vCnZoneYearArray = setnan(amsreBt07vCnYearArray, ~zonesLcIndexArray);
        ...
        % ------------------------------------------------------------------------------------------
        % Regress the zonal AMSR2 BT and MODIS LST by year, and get the retreived AMSR2 LST images.
        if flg5 == 1 || flg5 == 4
            ...
            amsreBt07hMaskZoneYearVector = amsreBt07hMaskZoneYearArray(valueIndex);
            amsreBt07vMaskZoneYearVector = amsreBt07vMaskZoneYearArray(valueIndex);
            ...
            if flg5 == 1 && flg3 == 2
                amsreBtsMaskZoneYearRecords = [amsreBt06hMaskZoneYearVector, ...
                    amsreBt06vMaskZoneYearVector, amsreBt07hMaskZoneYearVector, ...
                    amsreBt07vMaskZoneYearVector, amsreBt10hMaskZoneYearVector, ...
            ...
            if modisLstMaskZoneYearVectorN >= 2
                ...
                if flg6 == 2
                    ...
                    if flg5 == 1 && flg3 == 2
                        amsreLstCnZoneYearArray = pYear2(17) + ...
                            pYear2(1) .* amsreBt06hCnZoneYearArray + ...
                            pYear2(2) .* amsreBt06vCnZoneYearArray + ...
                            pYear2(3) .* amsreBt07hCnZoneYearArray + ...
                            pYear2(4) .* amsreBt07vCnZoneYearArray + ...
                            pYear2(5) .* amsreBt10hCnZoneYearArray + ...
                            pYear2(6) .* amsreBt10vCnZoneYearArray + ...
                            pYear2(7) .* amsreBt18hCnZoneYearArray + ...
                            pYear2(8) .* amsreBt18vCnZoneYearArray + ...
                            pYear2(9) .* amsreBt23hCnZoneYearArray + ...
                            pYear2(10) .* amsreBt23vCnZoneYearArray + ...
                            pYear2(11) .* amsreBt36hCnZoneYearArray + ...
                            pYear2(12) .* amsreBt36vCnZoneYearArray + ...
                            pYear2(13) .* amsreBt89hCnZoneYearArray + ...
                            pYear2(14) .* amsreBt89vCnZoneYearArray + ...
                            pYear2(15) .* amsreBtQd1CnZoneYearArray + ...
                            pYear2(16) .* amsreBtQd2CnZoneYearArray;
                    else
            ...
            % Export the scatterplot of AMSR2 BT(LST) vs MODIS LST by year.
            if flg4 == 1
        ...
        % ------------------------------------------------------------------------------------------
        % Regress and scatter the LST and BT by season, and get the retreived AMSR2 LST images.
        if flg5 == 2 || flg5 == 4
            for i = 1 : 4
                ...
                amsreBt07hMaskZoneSeasonArray = amsreBt07hMaskZoneYearArray(:, :, seasonIndex);
                amsreBt07vMaskZoneSeasonArray = amsreBt07vMaskZoneYearArray(:, :, seasonIndex);
                ...
                if flg6 == 2
                    ...
                    amsreBt07hCnZoneSeasonArray = amsreBt07hCnZoneYearArray(:, :, seasonIndex);
                    amsreBt07vCnZoneSeasonArray = amsreBt07vCnZoneYearArray(:, :, seasonIndex);
                ...
                % Regress the zonal BT & LST by season, and get the retreived AMSR2 LST images.
                ...
                amsreBt07hMaskZoneSeasonVector = amsreBt07hMaskZoneSeasonArray(valueIndex);
                amsreBt07vMaskZoneSeasonVector = amsreBt07vMaskZoneSeasonArray(valueIndex);
                ...
                if flg5 == 2 && flg3 == 2
                    amsreBtsMaskZoneSeasonRecords = [amsreBt06hMaskZoneSeasonVector, ...
                        amsreBt06vMaskZoneSeasonVector, amsreBt07hMaskZoneSeasonVector, ...
                        amsreBt07vMaskZoneSeasonVector, amsreBt10hMaskZoneSeasonVector, ...
                ...
                if modisLstMaskZoneSeasonVectorN >= 2
                    ...
                    if flg6 == 2
                        ...
                        if flg5 == 2 && flg3 == 2
                            amsreLstCnZoneSeasonArray = pYear2(17) + ...
                                pYear2(1) .* amsreBt06hCnZoneSeasonArray + ...
                                pYear2(2) .* amsreBt06vCnZoneSeasonArray + ...
                                pYear2(3) .* amsreBt07hCnZoneSeasonArray + ...
                                pYear2(4) .* amsreBt07vCnZoneSeasonArray + ...
                                pYear2(5) .* amsreBt10hCnZoneSeasonArray + ...
                                pYear2(6) .* amsreBt10vCnZoneSeasonArray + ...
                                pYear2(7) .* amsreBt18hCnZoneSeasonArray + ...
                                pYear2(8) .* amsreBt18vCnZoneSeasonArray + ...
                                pYear2(9) .* amsreBt23hCnZoneSeasonArray + ...
                                pYear2(10) .* amsreBt23vCnZoneSeasonArray + ...
                                pYear2(11) .* amsreBt36hCnZoneSeasonArray + ...
                                pYear2(12) .* amsreBt36vCnZoneSeasonArray + ...
                                pYear2(13) .* amsreBt89hCnZoneSeasonArray + ...
                                pYear2(14) .* amsreBt89vCnZoneSeasonArray + ...
                                pYear2(15) .* amsreBtQd1CnZoneSeasonArray + ...
                                pYear2(16) .* amsreBtQd2CnZoneSeasonArray;
                        else
                ...
                % Export the scatterplot of AMSR2 BT(LST) vs MODIS LST by season.
                if flg4 == 1
                    % Create the path used for restoring the zonal scatterplot.
                    outFigureYearZonePath = [outFigureYearPath, zoneName, '\',];
                    if ~exist(outFigureYearZonePath, 'dir')
        ...
        % ------------------------------------------------------------------------------------------
        % Regress and scatter the LST and BT by month, and get the retreived AMSR2 LST images.
        if flg5 == 3 || flg5 == 4
            for i = 1 : 12
                ...
                amsreBt07hMaskZoneMonthArray = amsreBt07hMaskZoneYearArray(:, :, monthIndex);
                amsreBt07vMaskZoneMonthArray = amsreBt07vMaskZoneYearArray(:, :, monthIndex);
                ...
                if flg6 == 2
                    ...
                    amsreBt07hCnZoneMonthArray = amsreBt07hCnZoneYearArray(:, :, monthIndex);
                    amsreBt07vCnZoneMonthArray = amsreBt07vCnZoneYearArray(:, :, monthIndex);
                ...
                % Regress the zonal BT & LST by month, and get the retreived AMSR2 LST images.
                ...
                amsreBt07hMaskZoneMonthVector = amsreBt07hMaskZoneMonthArray(valueIndex);
                amsreBt07vMaskZoneMonthVector = amsreBt07vMaskZoneMonthArray(valueIndex);
                ...
                if flg3 == 2
                    amsreBtsMaskZoneMonthRecords = [amsreBt06hMaskZoneMonthVector, ...
                        amsreBt06vMaskZoneMonthVector, amsreBt07hMaskZoneMonthVector, ...
                        amsreBt07vMaskZoneMonthVector, amsreBt10hMaskZoneMonthVector, ...
                    ...
                    if flg6 == 2
                        ...
                        if flg3 == 1
                            amsreLstCnZoneMonthArray = pYear2(15) + ...
                                ...
                                pYear2(3) .* amsreBt07hCnZoneMonthArray + ...
                                pYear2(4) .* amsreBt07vCnZoneMonthArray + ...
                                pYear2(5) .* amsreBt10hCnZoneMonthArray + ...
                                pYear2(6) .* amsreBt10vCnZoneMonthArray + ...
                                pYear2(7) .* amsreBt18hCnZoneMonthArray + ...
                                pYear2(8) .* amsreBt18vCnZoneMonthArray + ...
                                pYear2(9) .* amsreBt23hCnZoneMonthArray + ...
                                pYear2(10) .* amsreBt23vCnZoneMonthArray + ...
                                pYear2(11) .* amsreBt36hCnZoneMonthArray + ...
                                pYear2(12) .* amsreBt36vCnZoneMonthArray + ...
                                pYear2(13) .* amsreBt89hCnZoneMonthArray + ...
                                pYear2(14) .* amsreBt89vCnZoneMonthArray;
                        elseif flg3 == 2
                            amsreLstCnZoneMonthArray = pYear2(17) + ...
                                pYear2(1) .* amsreBt06hCnZoneMonthArray + ...
                                pYear2(2) .* amsreBt06vCnZoneMonthArray + ...
                                pYear2(3) .* amsreBt07hCnZoneMonthArray + ...
                                pYear2(4) .* amsreBt07vCnZoneMonthArray + ...
                                ...
                                pYear2(13) .* amsreBt89hCnZoneMonthArray + ...
                                pYear2(14) .* amsreBt89vCnZoneMonthArray + ...
                                pYear2(15) .* amsreBtQd1CnZoneMonthArray + ...
                                pYear2(16) .* amsreBtQd2CnZoneMonthArray;
                        end
                ...
                % Export the scatterplot of AMSR2 BT(LST) vs MODIS LST by month.
                if flg4 == 1
                    % Create the path used for restoring the zonal scatterplot.
                    outFigureYearZonePath = [outFigureYearPath, zoneName, '\',];
                    if ~exist(outFigureYearZonePath, 'dir')
        ...
        if flg4 == 2  % flg4 should be set 0 and 1. 2 means doesn't export figures in any cases.
            ...
            for i = 1 : yearDateListN
                ...
                amsreBtMaskChannelList = dir([amsreBtMaskDayPath,'AMSR2_D*',orbits{flg1},'*.tif']);
...
%             coefficientMonthArray(zonesLcIdList == 6007, :, [1, 2, 3, 12]);
    ...
    % ----------------------------------------------------------------------------------------------
    % Retrieve AMSR2 LST in pure zones(1:1:71, 1000:100:2000, 3000:1000:6000) using optimized model.
    outAmsreLstPureMatPath = [matlabVariablePath, '6_AMSR2_LST_PureLc_', regressModels{flg3}, ...
        dayNight{flg1}, '_', testYears{flg7}, '.mat'];
...
%         fixedCoefficientMonthArray = coefficientMonthArray;
        ...
        for n = 1 : pureLcIdN
            zoneName = ['Zone', num2str(zonesLcIdList(n))];
            disp(['Retrieve AMSR2 LST in pure pixels: ', zoneName]);
            ...
            for i = 1 : 12  % Month.
                ...
                if flg3 == 1
                    amsreLstCnZoneMonthArray = pOptimized(end) + ...
                        ...
                        pOptimized(3) .* amsreBt07hCnZoneMonthArray + ...
                        pOptimized(4) .* amsreBt07vCnZoneMonthArray + ...
                        pOptimized(5) .* amsreBt10hCnZoneMonthArray + ...
                        pOptimized(6) .* amsreBt10vCnZoneMonthArray + ...
                        pOptimized(7) .* amsreBt18hCnZoneMonthArray + ...
                        pOptimized(8) .* amsreBt18vCnZoneMonthArray + ...
                        pOptimized(9) .* amsreBt23hCnZoneMonthArray + ...
                        pOptimized(10) .* amsreBt23vCnZoneMonthArray + ...
                        pOptimized(11) .* amsreBt36hCnZoneMonthArray + ...
                        pOptimized(12) .* amsreBt36vCnZoneMonthArray + ...
                        pOptimized(13) .* amsreBt89hCnZoneMonthArray + ...
                        pOptimized(14) .* amsreBt89vCnZoneMonthArray;
                elseif flg3 == 2
                    amsreLstCnZoneMonthArray = pOptimized(end) + ...
                        pOptimized(1) .* amsreBt06hCnZoneMonthArray + ...
                        pOptimized(2) .* amsreBt06vCnZoneMonthArray + ...
                        ...
                        pOptimized(13) .* amsreBt89hCnZoneMonthArray + ...
                        pOptimized(14) .* amsreBt89vCnZoneMonthArray + ...
                        pOptimized(15) .* amsreBtQd1CnZoneMonthArray + ...
                        pOptimized(16) .* amsreBtQd2CnZoneMonthArray;
                end
                amsreLstCnZoneMonthArray(isnan(amsreLstCnZoneMonthArray)) = 0;
    ...
    % Retrieve the AMSR2 LST in mixed Lc pixels.
    ...
    outAmsreLstMixedMatPath = [matlabVariablePath, '7_AMSR2_LST_MixedLc_', regressModels{flg3}, ...
        dayNight{flg1}, '_', testYears{flg7}, '.mat'];
    if ~exist(outAmsreLstMixedMatPath, 'file')
        ...
        for n = 1 : zonesIdListN
            ...
            disp(['Retrieve AMSR2 LST in mixed pixels: ', num2str(mixLcId)]);
            ...
            % Get the complete AMSR2 BT images of the zone in 12 channels throughout the year.
            ...
            amsreBt07hCnMixPixelYearArray = setnan(amsreBt07hCnYearArray, ~mixPixelIndexArray);
            amsreBt07vCnMixPixelYearArray = setnan(amsreBt07vCnYearArray, ~mixPixelIndexArray);
            ...
            % Derive AMSR2 BT in mixed pixels of each zone by month.
            for i = 1 : 12
                monthIndex = ismember(modisLstMaskMonthList, i);
                monthIndexN = sum(monthIndex);
                ...
                % Get the complete AMSR2 BT images of the zone in the month.
                ...
                amsreBt07hCnMixPixelMonthArray = amsreBt07hCnMixPixelYearArray(:, :, monthIndex);
                amsreBt07vCnMixPixelMonthArray = amsreBt07vCnMixPixelYearArray(:, :, monthIndex);
                ...
                % Find the regression coefficients of AMSR2 LST in certain land cover.
...
%                 pBuildingMonth = fixedCoefficientMonthArray(zonesLcIdList == 10, :, i);
                ...
                % Regression.
                % First combine the coefficients, then multiply it by AMSR2 BTs in 12 channels.
                ...
                if flg3 == 1
                    amsrePartialLstCnMixedLcMonthArray = coefficientsAmsreBt(:, :, :, end) + ...
                        ...
                        coefficientsAmsreBt(:, :, :, 3) .* amsreBt07hCnMixPixelMonthArray + ...
                        coefficientsAmsreBt(:, :, :, 4) .* amsreBt07vCnMixPixelMonthArray + ...
                        coefficientsAmsreBt(:, :, :, 5) .* amsreBt10hCnMixPixelMonthArray + ...
                        coefficientsAmsreBt(:, :, :, 6) .* amsreBt10vCnMixPixelMonthArray + ...
                        coefficientsAmsreBt(:, :, :, 7) .* amsreBt18hCnMixPixelMonthArray + ...
                        coefficientsAmsreBt(:, :, :, 8) .* amsreBt18vCnMixPixelMonthArray + ...
                        coefficientsAmsreBt(:, :, :, 9) .* amsreBt23hCnMixPixelMonthArray + ...
                        coefficientsAmsreBt(:, :, :, 10) .* amsreBt23vCnMixPixelMonthArray + ...
                        coefficientsAmsreBt(:, :, :, 11) .* amsreBt36hCnMixPixelMonthArray + ...
                        coefficientsAmsreBt(:, :, :, 12) .* amsreBt36vCnMixPixelMonthArray + ...
                        coefficientsAmsreBt(:, :, :, 13) .* amsreBt89hCnMixPixelMonthArray + ...
                        coefficientsAmsreBt(:, :, :, 14) .* amsreBt89vCnMixPixelMonthArray;
                elseif flg3 == 2
                    amsrePartialLstCnMixedLcMonthArray = coefficientsAmsreBt(:, :, :, end) + ...
                        coefficientsAmsreBt(:, :, :, 1) .* amsreBt06hCnMixPixelMonthArray + ...
                        coefficientsAmsreBt(:, :, :, 2) .* amsreBt06vCnMixPixelMonthArray + ...
                        ...
                        coefficientsAmsreBt(:, :, :, 13) .* amsreBt89hCnMixPixelMonthArray + ...
                        coefficientsAmsreBt(:, :, :, 14) .* amsreBt89vCnMixPixelMonthArray + ...
                        coefficientsAmsreBt(:, :, :, 15) .* amsreBtQd1CnMixPixelMonthArray + ...
                        coefficientsAmsreBt(:, :, :, 16) .* amsreBtQd2CnMixPixelMonthArray;
                end
    ...
    outAmsreLstDesertMatPath = [matlabVariablePath, '8_AMSR2_LST_Desert_', regressModels{flg3}, ...
        dayNight{flg1}, '_', testYears{flg7}, '.mat'];
    if ~exist(outAmsreLstDesertMatPath, 'file')
        ...
        for n = 1 : desertCodeListN
            ...
            disp(['Retrieve AMSR2 LST in rest desert part: ', num2str(desertId)]);
            ...
            amsreBt07hCnDesertYearArray = setnan(amsreBt07hCnYearArray, ~mixPixelInDesertIndexArray);
            amsreBt07vCnDesertYearArray = setnan(amsreBt07vCnYearArray, ~mixPixelInDesertIndexArray);
            ...
            for i = 1 : 12  % Month.
                ...
                amsreBt07hCnDesertMonthArray = amsreBt07hCnDesertYearArray(:, :, monthIndex);
                amsreBt07vCnDesertMonthArray = amsreBt07vCnDesertYearArray(:, :, monthIndex);
                ...
                % Find the regression coefficients of AMSR2 LST in desert. Desert 1200 has no 
                ...
                if flg3 == 1
                    amsreLstCnPartial2MonthArray = ...
                        ...
                        pDesertMonth(3) .* amsreBt07hCnDesertMonthArray + ...
                        pDesertMonth(4) .* amsreBt07vCnDesertMonthArray + ...
                        pDesertMonth(5) .* amsreBt10hCnDesertMonthArray + ...
                        pDesertMonth(6) .* amsreBt10vCnDesertMonthArray + ...
                        pDesertMonth(7) .* amsreBt18hCnDesertMonthArray + ...
                        pDesertMonth(8) .* amsreBt18vCnDesertMonthArray + ...
                        pDesertMonth(9) .* amsreBt23hCnDesertMonthArray + ...
                        pDesertMonth(10) .* amsreBt23vCnDesertMonthArray + ...
                        pDesertMonth(11) .* amsreBt36hCnDesertMonthArray + ...
                        pDesertMonth(12) .* amsreBt36vCnDesertMonthArray + ...
                        pDesertMonth(13) .* amsreBt89hCnDesertMonthArray + ...
                        pDesertMonth(14) .* amsreBt89vCnDesertMonthArray);
                elseif flg3 == 2
                    amsreLstCnPartial2MonthArray = ...
                        desertPercentDesertMonthArray .* (pDesertMonth(end) + ...
                       pDesertMonth(1) .* amsreBt06hCnDesertMonthArray + ...
                        ...
                        pDesertMonth(13) .* amsreBt89hCnDesertMonthArray + ...
                        pDesertMonth(14) .* amsreBt89vCnDesertMonthArray + ...
                        pDesertMonth(15) .* amsreBtQd1CnDesertMonthArray + ...
                        pDesertMonth(16) .* amsreBtQd2CnDesertMonthArray);
                end
...
outAmsreLstCnMatPath = [matlabVariablePath, '9_AMSR2_LST_TP_', regressModels{flg3}, ...
    dayNight{flg1}, '_', testYears{flg7}, '.mat'];
...
% Remove the outlier of retrieved AMSR2 LST.


%% Patch the gaps in AMSR2 LST images.
% The days interval for interpolate the AMSR2 LST in orbit gaps.
% outAmsreLstFillMatPath = ...
%     [matlabVariablePath, '10_AMSR2_LST_Filled_', regressModels{flg3}, testYears{flg7}, '.mat'];
% if ~exist(outAmsreLstFillMatPath, 'file')
%     interpDays = 9;
%     halfInterval = floor(interpDays / 2);
%     
%     % Add the head and tail layers to the AMSR2 LST array.
...
%     
%     % Fill the gaps in the daily AMSR2 LST images by interpolating the timeseries of the pixels
...
if flg4 == 1
    for i = 1 : yearDateListN
        ...
        % Merged AMSR2 LST for China.
        outAmsreLstImagePath = [outAmsreLstYearPath, 'AMSR2_LST_merged_', testDate,  '_', dayNight{flg1},'.tif'];
        if ~exist(outAmsreLstImagePath, 'file')
            amsreLstCnLayer = amsreLstCnYearArray(:, :, i);
            geotiffwrite(outAmsreLstImagePath, amsreLstCnLayer, btLstReference);
        end
        
%         % Gap-filled AMSR2 LST for China.
%         outAmsreLstImagePath = [outAmsreLstYearPath, 'AMSR2_LST_filled_', testDate, '_Day.tif'];
%         if ~exist(outAmsreLstImagePath, 'file')
...
%         end
    ...
    % AMSR2 and MODIS LST scatterplots for the extent of China in the whole year.
    outLstScatterYearPath = [outFigureYearPath, 'China', '_Regressed_', testYears{flg7},  '_', dayNight{flg1}, '.png'];
    if ~exist(outLstScatterYearPath, 'file')
...
% % Export Scatterplot between AMSR2 LST and MODIS LST based on the zonal strategy of Zhou.
...
% Scatter AMSR2 BT and MODIS LST.
...
xlabel('AMSR2 BT'); ylabel('MODIS LST');
title(['AMSR2 89V BT vs MODIS LST in ', zoneName, ' ', dateString]);
...
% Scatter AMSR2 LST and MODIS LST.
...
title(['AMSR2 LST vs MODIS LST in ', zoneName, ' ', dateString]);
...
title(['RMSE of retrieved AMSR2 LST in 2010 ', period]);
end
