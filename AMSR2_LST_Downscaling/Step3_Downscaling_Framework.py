...
regionYearDaynight = '{0}_{1}_{2}'.format(region, yearNum, dayNight)
...
regionStaCsvName = 'DateSelect_{0}.csv'.format(regionYearDaynight)
regionStaCsvPath = path.join(regionDir, regionStaCsvName)
with open(regionStaCsvPath, 'r') as f1:
...
# 保存降尺度后LST精度评价统计文件.
regionStaCsvName2 = 'Downscaling_{0}_{1}.csv'.format(regionYearDaynight, sourceLst, strategy)
regionStaCsvPath2 = path.join(regionDir, regionStaCsvName2)

#%% 遍历有效数据日期, 并进行降尺度.
errorRecordList = []
regionYearDir = path.join(regionDir, regionYearDaynight)
...
# 保存精度评价统计文件.
with open(regionStaCsvPath2, 'w') as f2:
